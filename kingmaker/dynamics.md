...
### Avor — Pax
- **Hobgoblin (Elfbane) Thief Rogue · dual-wield Aldori Dueling Sword + Shortsword · deity Dahak**
- Off-guard skirmisher; fear-lockdown engine (Intimidating Glare → Dread Striker @L4 → Remorseless Lash). Party scout/trapfinder and a second Face (Deception/Intimidation). Leans team-utility (Scout archetype planned).
- Build: Foundry export `fvtt-Actor-avor-MdKE7WI6EpSidkxm.json`.
...
- **Tripkee Commander**
- Tactical support: auras, action-granting, buffs the martials; INT knowledge (Arcana/Occultism/Society). Plays **mid-line/flexible** — don't assume he's always in melee to complete a flank.

### Caleb — *(player TBD)*
- **Human Cleric (Erastil) — divine caster + longbow** *(confirmed in S4 roll log)*
- Healer and the party's **only** caster. In S4: **Heal** (line-healing), **Void Warp** (offensive void cantrip), **Bless** (+1 to-hit aura), and **fires Arrows** — so he pulls double duty as ranged support. Reads as Warpriest-flavored (verify vs. Cloistered).

## Synergies to lean on (Avor)

- **Flank with Soft Willow** — the pincer that powers Sneak Attack at levels 1–3 (before Dread Striker).
- **Open with Surprise Attack** — roll Stealth/Deception for initiative → round-1 off-guard for you *and* the Swashbuckler; amplified if you take Scout (party goes first).
- **Fear helps everyone** — even pre-Dread-Striker, your Demoralize is a −status debuff the whole team cashes in.

## GM — Luke Santos (early read)
